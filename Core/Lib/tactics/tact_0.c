...
tact_0 ()
{
	...
	case 11:
		...
		break;
	case 20:
		cur_task = move_to_xy (x_side (735), -700, FORWARD, 0.2, W_MAX_DEF,
				NO_SENS);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 30;
		break;
	case 30:
		...
		break;
	case 40:
		cur_task = move_to_xy (x_side (1185), -605, BACKWARD, V_MAX_DEF, W_MAX_DEF, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 50;
		break;
	case 50: //krece ka ms23
		cur_task = move_to_xy (x_side (1300), -605, BACKWARD, 0.2,
				W_MAX_DEF,
				NO_SENS);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 60;
	...
	case 70: //krece ka ca6
		cur_task = move_on_path (x_side (1120), -220, phi_side (45), FORWARD, 0, 0.5, 0, FORWARD);
	...
	cur_task = move_on_path (x_side (480), 250, phi_side (0), BACKWARD, 0, 0.5, 0,
			BACKWARD);
	if (cur_task == TASK_SUCCESS)
		tact_fsm_case = 91;
	if (timeout (5000))
	...
	case 110:
		...
		tact_fsm_case = 120;
		break;
	case 120: //krece ka ms21
		cur_task = move_on_path (x_side (1130), 290, phi_side (0), FORWARD, 0, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 130;
		break;
		//	case 125:
		//		cur_task = move_to_xy(x_side (950), 350, FORWARD, V_MAX_DEF*0.3, W_MAX_DEF*0.5, FORWARD);
		//		if (cur_task == TASK_SUCCESS)
		//			tact_fsm_case = 130;
		//		break;
	case 130:
	...
	case 140:
		...
		break;
	case 150:
		cur_task = move_on_dir (150, FORWARD, 0.12, NO_SENS);
		if (cur_task == TASK_SUCCESS || timeout (3000))
		{
			tact_fsm_case = 160;
			reset_all_delays ();
	...
	case 160:
		...
		break;
	case 170:
		cur_task = move_on_dir (500, BACKWARD, 1.0, NO_SENS);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 180;
		break;

	case 180:
		cur_task = move_to_xy (x_side (820), -170, FORWARD, V_MAX_DEF, W_MAX_DEF * 0.5, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 190;
		break;
	...
	case 195:
		cur_task = move_on_dir (80, FORWARD, 1.0, NO_SENS);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 200;
		break;
	...
	case 210:
		cur_task = move_to_xy (x_side (0), -350, FORWARD, V_MAX_DEF, W_MAX_DEF, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 220;
		break;

	case 220:
		cur_task = move_to_xy (x_side (-800), 150, FORWARD, V_MAX_DEF, W_MAX_DEF, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 9999;
		break;
		...
		/*case 120: //krece ka ms14
			 ...
			 cur_task = move_on_path (x_side (-900), 0, phi_side (-90), FORWARD, 1, 0.51, 0,
			 FORWARD);
			 ...
			 break;
			 case 230:
			 cur_task = move_on_path (x_side (-650), 400, phi_side (-90), BACKWARD, 0, 0.51, 0, BACKWARD);
			 if (cur_task == TASK_SUCCESS)
			 tact_fsm_case = 240;
			 break;
			 case 240:
			 ...
			 break;
			 case 280:
			 cur_task = move_on_path (x_side (-700), -800, phi_side (90), BACKWARD, 0, 0.51, 0, BACKWARD);
			 if (cur_task == TASK_SUCCESS)
			 tact_fsm_case = 285;
			 break;
			 case 285:
			 ...
			 case 215:
			 cur_task = rot_to_phi (phi_side (135), W_MAX_DEF, NO_SENS);
			 if (cur_task == TASK_SUCCESS)
			 tact_fsm_case = 220;
			 break;
		...
		//		case 500: //ca5
		//			cur_task = rot_to_xy (x_side (1250), -760, BACKWARD, W_MAX_DEF, NO_SENS);
		//			if (cur_task == TASK_SUCCESS)
		//				tact_fsm_case = 502;
		//			break;
	case 500:
		cur_task = move_to_xy (x_side (1250), -720, BACKWARD, V_MAX_DEF, W_MAX_DEF, NO_SENS);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 505;
		break;
	case 505:
		...
		break;
	case 507:
		cur_task = move_on_dir (40, BACKWARD, 1.0, NO_SENS);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 510;
		break;
	case 510:
	...
	case 511:
		...
		break;
	case 512:
		cur_task = move_on_dir (220, BACKWARD, 0.2, NO_SENS);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 518;
		break;

	case 518:
		cur_task = move_on_dir (200, FORWARD, 0.5, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 520;
		break;
	case 520: // krece ka ms12
	cur_task = move_to_xy (x_side (-380), -380, FORWARD, V_MAX_DEF, W_MAX_DEF, FORWARD);
	if (cur_task == TASK_SUCCESS)
		tact_fsm_case = 525;
	if (timeout (5000))
		tact_fsm_case = 10000;
	...
	case 530:
		...
		break;
	case 540:
		cur_task = move_on_dir (250, FORWARD, 0.2, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 560;
		break;
	case 560:
	...
	case 570: // krece ka ms14
		cur_task = move_on_path (x_side (-675), -400, phi_side (45), BACKWARD, 0, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 580;
		/*if (timeout(5000))
			 tact_fsm_case = 15000;*/
	...
	case 590:
		...
		break;
	case 600:
		cur_task = move_on_dir (250, BACKWARD, 0.2, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 610;
		break;
	case 610:
		...
		break;
	case 620: // Baner
	cur_task = move_to_xy (x_side (-750), -760, BACKWARD, 0.5, W_MAX_DEF, NO_SENS);
	if (cur_task == TASK_SUCCESS)
		tact_fsm_case = 630;
	break;
	...
	case 700:
		cur_task = move_on_dir (200, BACKWARD, V_MAX_DEF, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 710;
		break;
	case 710:
	...
	case 1015: //krece ka ca4
		cur_task = move_on_path (x_side (-300), -400, phi_side (90), BACKWARD, 0, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 1019;
		break;
	case 1019:
	...
	case 1025: //krece ka ms12
		cur_task = move_on_path (x_side (-400), -200, phi_side (-90), BACKWARD, 0, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 1028;
	...
	case 1050: //krece ka ms21
		cur_task = move_on_path (x_side (600), 350, phi_side (180), BACKWARD, 1, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 1060;
		if (cur_task == TASK_FAIL)
			tact_fsm_case = 3500;
		break;
	case 1060:
		cur_task = move_on_path (x_side (1300), 350, phi_side (180), BACKWARD, 0, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 1065;
		break;
	case 1065:
	...
	case 1180: //krece ka kraju
		cur_task = move_on_path (x_side (0), 200, phi_side (0), BACKWARD, 1, 0.51, 0,
				BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 1190;
		break;
	case 1190:
		cur_task = move_on_path (x_side (-1100), 800, phi_side (-90), BACKWARD, 0, 0.51, 0,
				BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = -1;
		break;
	...
	case 1500: //krece ka ms14
		cur_task = move_on_path (x_side (-300), -400, phi_side (90), BACKWARD, 1, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 1510;
		break;
	case 1510:
		cur_task = move_on_path (x_side (-700), -550, phi_side (90), BACKWARD, 0, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 1515;
		break;
	case 1514:
	...
	case 1525:
		...
		break;
	case 1530: //gradi
	cur_task = task_sprat_1 (BACKWARD, IN_GRTL);
	if (cur_task == TASK_SUCCESS)
		tact_fsm_case = 1540;
	break;
	case 1540: //krece ka ms12
		cur_task = move_on_path (x_side (-400), -200, phi_side (-90), FORWARD, 0, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 1550;
		if (cur_task == TASK_FAIL)
			tact_fsm_case = 2000;
	...
	case 1560:
		...
		break;
	case 1570: //baner
		cur_task = move_on_path (x_side (1450), 300, phi_side (0), FORWARD, 0, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 1580;
		if (cur_task == TASK_FAIL)
			tact_fsm_case = 3000;
	...
	case 2000:
		cur_task = move_on_path (x_side (-300), -700, phi_side (90), BACKWARD, 1, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 2010;
		break;
	case 2010: //krece ka ms22
		cur_task = move_on_path (x_side (400), 200, phi_side (90), FORWARD, 0, 0.51, 0,
				FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 2014;
		if (cur_task == TASK_FAIL)
	...
	case 2500: //krece ka ms10
		...
		break;
	case 2510:
		cur_task = move_on_path (x_side (-650), 600, phi_side (-90), FORWARD, 0, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 2515;
		break;
	case 2515:
	...
	case 2540:
		...
		break;
	case 2550:
		cur_task = move_on_path (x_side (0), 200, phi_side (0), FORWARD, 1, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 2510;
		break;
	case 2560:
		cur_task = move_on_path (x_side (-1450), 0, phi_side (0), FORWARD, 0, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 2570;
		break;
	case 2570: //baner
	...
	case 3000: //baner krece
		...
		break;
	case 3010:
		cur_task = move_on_path (x_side (1450), -600, phi_side (180), BACKWARD, 0, 0.51, 0,
				BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 3020;
		break;
	...
	case 3110: //krece ka kraju
		cur_task = move_on_path (x_side (0), 200, phi_side (0), FORWARD, 1, 0.51, 0,
				FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 3120;
		break;
	case 3120:
		cur_task = move_on_path (x_side (-1100), 800, phi_side (-90), FORWARD, 0, 0.51, 0,
				FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = -1;
		break;
	...
	case 3500:
		cur_task = move_on_path (x_side (-400), 0, phi_side (-90), FORWARD, 1, 0.51, 0,
				FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 3510;
		break;
	case 3510:
		cur_task = move_on_path (x_side (1450), -500, phi_side (0), FORWARD, 0, 0.51, 0,
				FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 3520;
		break;
	...
	case 3580: //krece ka ms10
		cur_task = move_on_path (x_side (0), 200, phi_side (180), FORWARD, 1, 0.51, 0,
				FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 3590;
		if (cur_task == TASK_FAIL)
	...
	case 3630: //krece ka ca4
		...
		break;
	case 3640:
		cur_task = move_on_path (x_side (-300), -850, phi_side (90), BACKWARD, 0, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 3645;
		if (cur_task == TASK_FAIL)
			tact_fsm_case = 6200;
	...
	case 3700: //krece ka ms14
		...
		break;
	case 3710:
		cur_task = move_on_path (x_side (-700), -550, phi_side (0), BACKWARD, 0, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 3715;
		break;
	case 3715:
	...
	case 3800: //krece ka ms22
		cur_task = move_on_path (x_side (400), 100, phi_side (90), BACKWARD, 1, 0.51, 0,
				BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 3802;
		break;
	...
	case 3820:
		...
		break;
	case 3830: //ca4/1
	cur_task = move_on_path (x_side (1300), -400, phi_side (0), FORWARD, 1, 0.51, 0,
			FORWARD);
	if (cur_task == TASK_SUCCESS)
		tact_fsm_case = 3840;
	break;
	...
	case 4000: //KRECE KA CA6/1
		cur_task = move_on_path (x_side (1300), 350, phi_side (0), FORWARD, 0, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 4010;
		break;
	case 4010:
	...
	case 4080: //krece ka ms12
		cur_task = move_on_path (x_side (-400), 100, phi_side (-90), FORWARD, 0, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 4085;
		if (cur_task == TASK_FAIL)
			tact_fsm_case = 5000;
	...
	case 4120: //KRECE KA MS10
		cur_task = move_on_path (x_side (-650), 600, phi_side (-90), BACKWARD, 0, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 4125;
		if (cur_task == TASK_FAIL)
			tact_fsm_case = 5500;
	...
	case 4150: //krece ka ca4
		...
		break;
	case 4160:
		cur_task = move_on_path (x_side (-300), -850, phi_side (90), FORWARD, 0, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 4165;
		break;
	case 4165:
	...
	case 4500: //krece ka ms14
		...
		break;
	case 4505:
		cur_task = move_on_path (x_side (-700), -550, phi_side (180), FORWARD, 0, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 4507;
		break;
	case 4507:
	...
	cur_task = move_on_path (x_side (-900), 0, phi_side (90), BACKWARD, 1, 0.51, 0, BACKWARD);
	if (cur_task == TASK_SUCCESS)
		tact_fsm_case = 4670;
	if (cur_task == TASK_FAIL)
		tact_fsm_case = 7000;
	...
	case 5120: // krece ka kraju
		...
		break;
	case 5130:
		cur_task = move_on_path (x_side (-1100), 800, phi_side (90), FORWARD, 0, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = -1;
		break;
	...
	case 5210:
		...
		break;
	case 5220:
		cur_task = move_on_path (x_side (-300), -850, phi_side (180), FORWARD, 0, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 5225;
		break;
	case 5225:
	...
	case 5500: //krece ka ms13
		...
		break;
	case 5510:
		cur_task = move_on_path (x_side (-1300), -600, phi_side (0), FORWARD, 180, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 5515;
		if (cur_task == TASK_FAIL)
			tact_fsm_case = 5700;
	...
	cur_task = move_on_path (x_side (-300), -700, phi_side (-90), FORWARD, 0, 0.51, 0, FORWARD);
	if (cur_task == TASK_SUCCESS)
		tact_fsm_case = 5555;
	if (cur_task == TASK_FAIL)
		tact_fsm_case = 6000;
	...
	case 5700: //KRECE KA MS10
		cur_task = move_on_path (x_side (400), 0, phi_side (-90), BACKWARD, 1, 0.51, 0,
				BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 5710;
		break;
	...
	case 5900: //krece ka ca4
		cur_task = move_on_path (x_side (0), 200, phi_side (180), BACKWARD, 1, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 5910;
		break;
	case 5910: //ca5
	...
	case 5920: //krece ka ca4
		cur_task = move_on_path (x_side (-300), -850, phi_side (0), BACKWARD, 0, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 5925;
		break;
	case 5925:
	...
	case 6000: //krece ka ca6/1
		cur_task = move_on_path (x_side (-1300), -600, phi_side (0), BACKWARD, 1, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 6010;
		break;
	case 6010:
	...
	case 6020:
		...
		break;
	case 6025:
		cur_task = move_on_path (x_side (1300), 0, phi_side (0), FORWARD, 1, 0.51, 0,
				FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 6030;
		break;
	...
	case 6100:
		cur_task = move_on_path (x_side (-650), 600, phi_side (90), FORWARD, 1, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 6110;
		break;
	...
	case 6300: //krece ka ms13
		cur_task = move_on_path (x_side (-700), -550, phi_side (90), BACKWARD, 1, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 6310;
		if (cur_task == TASK_FAIL)
			tact_fsm_case = 6600;
		break;
	case 6310:
		cur_task = move_on_path (x_side (-1300), -600, phi_side (180), FORWARD, 0, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 6320;
		break;
	case 6320:
	...
	case 6600: // krece ka ms10
		...
		break;
	case 6610:
		cur_task = move_on_path (x_side (-650), 600, phi_side (180), FORWARD, 0, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 6620;
		break;
	case 6620:
	...
	case 6800:
		cur_task = move_on_path (x_side (-1300), -600, phi_side (180), FORWARD, 1, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 6810;
		break;
	...
	case 6830: //KRECE KA CA6/1
		cur_task = move_on_path (x_side (1350), 100, phi_side (90), BACKWARD, 0, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 6840;
		break;
	case 6840:
	...
	case 10030:
		...
		break;
	case 10040:
		cur_task = move_to_xy (x_side (-675), -600, BACKWARD, V_MAX_DEF, W_MAX_DEF, NO_SENS);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 10050;
		break;
	case 10050:
	...
	case 10100000: //ms14
	cur_task = move_on_path (x_side (-700), -550, phi_side (-90), FORWARD, 0, 0.51, 0, FORWARD);
	if (cur_task == TASK_SUCCESS)
		tact_fsm_case = 10110;
	if (cur_task == TASK_FAIL)
		tact_fsm_case = 12200;
	...
	case 10560: //krece ka ms11
		cur_task = move_on_path (x_side (-1300), 350, phi_side (0), BACKWARD, 0, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 10570;
		if (cur_task == TASK_FAIL)
			tact_fsm_case = 10800;
	...
	case 10800: //krece ka ms21
		cur_task = move_on_path (x_side (-650), 600, phi_side (0), FORWARD, 1, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 10810;
		break;
	case 10810:
	...
	case 11200: //krece ka ca4
		cur_task = move_on_path (x_side (-1300), 350, phi_side (0), BACKWARD, 1, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 11210;
		break;
	case 11210:
		...
		break;
	case 11220:
		cur_task = move_on_path (x_side (-300), -800, phi_side (-90), FORWARD, 0, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 11230;
		break;
	case 11230:
	...
	case 11500:
		cur_task = move_on_path (x_side (1100), -100, phi_side (180), BACKWARD, 1, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 11510;
		break;
	...
	case 12000:
		cur_task = move_on_path (x_side (-700), -800, phi_side (-90), FORWARD, 0, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 12010;
		break;
	case 12010:
	...
	case 12100:
		cur_task = move_on_path (x_side (-300), -700, phi_side (90), BACKWARD, 0, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 12110;
		break;
	...
	case 12110:
		...
		break;
	case 12120: //gradi
		cur_task = task_sprat_1 (FORWARD, IN_GRTL);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 12130;
		break;
	...
	case 12210: //krece ka ms11
		cur_task = move_on_path (x_side (-1300), 350, phi_side (180), FORWARD, 0, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 12220;
		break;
	case 12220:
	...
	case 12250: //krece ka ca6
		cur_task = move_on_path (x_side (100), -100, phi_side (180), BACKWARD, 0, 0.51, 0,
				BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 12260;
		if (cur_task == TASK_FAIL)
	...
	case 15040:
		...
		tact_fsm_case = 15050;
		break;
	case 15050: //krece ka ca6
		cur_task = move_on_path (x_side (820), -120, phi_side (45), FORWARD, 0, 0.5, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 15060;
		break;
	case 15060:
	...
	case 15080:
		...
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 15100;
		break;
	case 15100: //krece ka ms10
		cur_task = move_on_path (x_side (0), 200, phi_side (180), FORWARD, 1, 0.51, 0, FORWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 15110;
		if (cur_task == TASK_FAIL)
			tact_fsm_case = 15500;
	...
	case 15240:
		...
		if (delay_nb_2 (&tact_delay_1, 500))
			tact_fsm_case = 15250;
		break;
	case 15260: //Gradi na ca4/1
	cur_task = task_sprat_3_1_full (BACKWARD);
	if (cur_task == TASK_SUCCESS)
		tact_fsm_case = 15270;
	break;
	case 15270: //krece ka kraju
		cur_task = move_on_path (x_side (-900), 0, phi_side (-90), BACKWARD, 1, 0.51, 0, BACKWARD);
		if (cur_task == TASK_SUCCESS)
			tact_fsm_case = 15280;
		break;
