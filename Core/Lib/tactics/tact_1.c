...
tact_1 ()
{
	switch (tact_fsm_case)
		...
		case 10: //srednji bunt blizi njemu
			cur_task = move_on_path (x_side (-360), -380, phi_side (135), FORWARD, 0, 0.5, 0, NO_SENS);
		...
		case 40: //bunt pored startnog polja
			if (get_tact_num_ptr ()->side)	// plava
				...
				cur_task = move_on_path (x_side (-700), -400, phi_side (45), BACKWARD, 0, 0.5, 0, NO_SENS);
		...
		case 70:
			if (get_tact_num_ptr ()->side)	// plava
				...
				cur_task = move_on_dir (450, FORWARD, 0.5, NO_SENS);
			if (cur_task == TASK_SUCCESS)
				tact_fsm_case = 80;
			break;
			/*case 70:
...
//				cur_task = task_sprat_3_1_full (FORWARD);
		...
		case 100:
			if (get_tact_num_ptr ()->side)	// plava
				...
				cur_task = move_on_dir (115, BACKWARD, 0.2, NO_SENS);
			if (cur_task == TASK_SUCCESS)
				tact_fsm_case = 110;
			break;
		case 110:
...
//			if (get_tact_num_ptr ()->side)	// plava
				cur_task = move_on_path (x_side (-705), 400, phi_side (90), FORWARD, 0, //-735
																	0.5, 0, FORWARD);
//			else
//				cur_task = move_on_path (x_side (-705), 430, phi_side (90), FORWARD, 0, 0.5, 0, FORWARD);
...
//				}
		...
		case 140:
			if (get_tact_num_ptr ()->side)	// plava
				cur_task = move_on_dir (150, FORWARD, 0.2, NO_SENS);	//140
			else
...
//				cur_task = task_sprat_3 (BACKWARD);
		...
		case 210: //zavrsetak
			cur_task = move_to_xy (x_side (-800), 150, FORWARD, 0.75, W_MAX_DEF,
			NO_SENS);
			if (cur_task == TASK_SUCCESS)
				tact_fsm_case = 9999;
			break;
		...
		case 510: //rezervisan bunt
			if (get_tact_num_ptr ()->side)	// plava
				...
				cur_task = move_to_xy (x_side (-705), 405, FORWARD, V_MAX_DEF * 0.5, W_MAX_DEF * 0.5, FORWARD); //losiji sto

			else
				//zuta strana
				cur_task = move_to_xy (x_side (-665), 350, BACKWARD, V_MAX_DEF * 0.5, W_MAX_DEF * 0.5, BACKWARD);
			if (cur_task == TASK_SUCCESS)
				tact_fsm_case = 520;
			break;
		case 520:
