...
tact_1 ()
{
	switch (tact_fsm_case)
		...
		case 160: //krece ka polju za gradnju
			cur_task = move_to_xy (x_side (-825), -600, BACKWARD, V_MAX_DEF,
			W_MAX_DEF,
															BACKWARD);
			if (cur_task == TASK_SUCCESS)
				tact_fsm_case = 170;
		...
		case 207: //zavrsetak
			cur_task = rot_to_xy (x_side (-600), -300, FORWARD, W_MAX_DEF, NO_SENS);
			if (cur_task == TASK_SUCCESS)
				tact_fsm_case = 208;
			break;
...
//			cur_task = move_to_xy (x_side (-600), -300, FORWARD, V_MAX_DEF, W_MAX_DEF, FORWARD);
		...
		case 510: //rezervisan bunt
			if (get_tact_num_ptr ()->side)	// plava
				...
				cur_task = move_to_xy (x_side (-705), 405, FORWARD, V_MAX_DEF * 0.3,
				W_MAX_DEF * 0.5,
																FORWARD); //losiji sto
		...
		case 595:
			...
			break;
		case 600:
			cur_task = move_to_xy (x_side (950), -100, FORWARD, V_MAX_DEF,
			W_MAX_DEF * 0.5,
															NO_SENS);
			if (cur_task == TASK_SUCCESS)
				tact_fsm_case = 610;
